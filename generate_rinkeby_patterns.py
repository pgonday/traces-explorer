...
LENDING_POOL_ADDRESSES_PROVIDER_ADDRESS = '0xA55125A90d75a95EC00130E8E8C197dB5641Eb19'
...
def load(action, address):
	...
	rsp = requests.get(f'https://api-rinkeby.etherscan.io/api?module=contract&action={action}&address={address}&apikey={apikey}', headers=headers)
...
lending_pool_addresses_provider = w3.eth.contract(address = LENDING_POOL_ADDRESSES_PROVIDER_ADDRESS, abi = load_json('pooladdressesprovider'))
lending_pool_address = lending_pool_addresses_provider.functions.getPool().call()
lending_pool = w3.eth.contract(address = lending_pool_address, abi = load_json('pool'))
...
for asset in list:
	...
	# variable debt token
	get_asset(reserve_data[10])


# Get contracts
get_contract(lending_pool_addresses_provider.address, 'PoolAddressesProvider')
get_contract(lending_pool.address, 'Pool')
get_contract(lending_pool_addresses_provider.functions.getPriceOracle().call(), 'AaveOracle')
